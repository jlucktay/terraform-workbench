...
parser = argparse.ArgumentParser(
    description='Generate ansible.cfg and dynamic ansible inventory')
parser.add_argument('--list', dest='list',
                    help='produce dynamic ansible inventory when called with a list', action='store_true')
parser.add_argument('--ssh', dest='ssh',
                    help='generate ssh config to use bastion', action='store_true')
parser.add_argument('--ssh-key', dest='ssh_key',
                    help='key to use for ansible', default=None)
parser.add_argument('--cluster', dest='cluster', help='specify default cluster name via env varialbe TF_VAR_cluster_name',
                    default=os.environ.get('TF_VAR_cluster_name', None))
